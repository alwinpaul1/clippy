...
Future<void> _handleRequest(HttpRequest req) async {
  ...
  // SEO / branding static files (allow-listed only).
  if (await _serveWebAsset(req)) return;
...
/// Public files under web/ that crawlers and social previews need.
const _webAssets = {
...
Future<bool> _serveWebAsset(HttpRequest req) async {
  final name = req.uri.path.startsWith('/')
      ? req.uri.path.substring(1)
      : req.uri.path;
  final mime = _webAssets[name];
  if (mime == null) return false;
  for (final dir in _webRoots) {
    final file = File('$dir/$name');
    if (file.existsSync()) {
      final parts = mime.split(';').first.split('/');
      req.response
        ..statusCode = HttpStatus.ok
...
///
/// If `GA_MEASUREMENT_ID` is set (e.g. `G-XXXXXXXX`), it replaces the
/// placeholder in the page so Google Analytics loads. Empty / unset = no GA.
